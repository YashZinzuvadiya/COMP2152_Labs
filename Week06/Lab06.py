...
def write_to_log(filename, entry):
    ...
    with open(filename, "a") as file:     # Open the file in append mode ("a") using a with statement
        file.write(entry + "\n")          # Write the entry + "\n" to the file
...
def log_to_csv(filename, command, target, result, status):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(filename, "a", newline="") as file: # Open filename in append mode ("a") with newline=""
...
def safe_read_log(filename):
    ...
    #     print "No log file found. Run a diagnostic first.
        ...
        print("Log read attempt completed")     #print "Log read attempt completed."
